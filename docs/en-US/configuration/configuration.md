...
easyscp supports a number of user-defined parameters. easyscp stores them in a
...
To enter the configuration, press `<CTRL+C>` from the easyscp home.

easyscp requires these paths to be accessible:

- `$HOME/.config/easyscp/` on Linux/BSD
- `$HOME/.config/easyscp/` on macOS
- `%USERPROFILE%\.easyscp\` on Windows

## Parameters

The following parameters can be configured:

- **Text Editor**: the text editor to use. By default easyscp finds the default
...
- **Default Protocol**: the default value for the file transfer protocol to be
  used in easyscp. It applies to the login page and to the address CLI argument.
- **Show Hidden Files**: whether hidden files are displayed by default. You can
  also toggle hidden files at runtime by pressing `A`.
- **Check for updates**: if set to `yes`, easyscp queries the GitHub API to
  check whether a new version of easyscp is available.
- **Prompt when replacing existing files**: if set to `yes`, easyscp prompts for
...
- **SSH configuration path**: SSH configuration file to use when connecting to a
  ...
  attributes supported by easyscp are listed at
  [the ssh2-config exposed attributes](https://crates.io/crates/ssh2-config#exposed-attributes).
