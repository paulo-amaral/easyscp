...
easyscp sends desktop notifications for the following events:
...
- **Transfer failed**: sent once a transfer has failed due to an error.
  Displayed only if the total transfer size is at least the configured
  `Notifications: minimum transfer size`.
- **Update available**: sent whenever a new version of easyscp is available.
- **Update installed**: sent whenever a new version of easyscp has been
  installed.
