...
disk at `$HOME/.config/easyscp`. The location protects the key with file
...
- For other desktop environments, you can use [KeepassXC](https://keepassxc.org/)
  to obtain a keyring. It must be set up to work with easyscp; see
  [KeepassXC setup](#keepassxc-setup) below.
- If you do not want to install any of these services, easyscp keeps working as
  usual and falls back to saving the key in a file, as it does for BSD and WSL.

### KeepassXC setup

Follow these steps to set up KeepassXC for easyscp:
...
6. Select "Secret service integration" and enable "Expose entries under this
   group".
7. Select the group where the easyscp secret will be kept. Note that any other
