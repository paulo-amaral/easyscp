...
Along with configuration, easyscp provides an essential feature for SFTP/SCP
...
- **Remove an existing key**: press `<DEL>` or `<CTRL+E>` on the key you want to
  remove to delete it persistently from easyscp.
...
authentication in easyscp is valid both for username/password authentication and
