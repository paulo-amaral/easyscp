...
easyscp lets you set the colors for several components in the application. There
are two ways to customize easyscp:
...
To customize easyscp from the configuration menu, enter the configuration from
...
one of the themes bundled with easyscp in the `themes/` directory of the
repository. Import a theme by running:

```sh
easyscp theme <theme_file>
```

If everything is fine, easyscp confirms the theme has been imported.

## Color syntax

easyscp accepts the following color formats:
...
2. Edit your theme by hand. If you use a custom theme, edit the file and add the
   ...
   `$CONFIG_DIR` is:
    ...
    - FreeBSD/Linux: `$HOME/.config/easyscp`
    - macOS: `$HOME/.config/easyscp`
    - Windows: `%USERPROFILE%\.easyscp`
