...
Welcome to the developer manual for easyscp. This chapter does NOT contain the
documentation for easyscp modules, which can instead be found on Rust Docs at
<https://docs.rs/easyscp>. This chapter describes how easyscp works and the
guidelines to implement features such as file transfers and additions to the
user interface.

easyscp is written in Rust (edition 2024, MSRV 1.89.0). The user interface is
built with [tuirealm](https://crates.io/crates/tuirealm) v3, which runs on top
of [crossterm](https://github.com/crossterm-rs/crossterm).

## How easyscp works

easyscp is basically made up of 3 core modules:
...
easyscp supports the following protocols: SFTP, SCP, FTP/FTPS, Kube, S3, SMB and
WebDAV.
...
- **Activities display Applications**: each activity can show different
  **Applications**. An application contains a **View**, which is basically a list
  ...
  more, read <https://crates.io/crates/tuirealm>.
...
- **Messages: an Elm-based approach**: input events are handled with an approach
  ...
  three basic functions: **update**, **view** and **init**. easyscp implements
...
easyscp implements a trait called `Activity`, a much reduced version of the
...
- `will_umount`: returns whether the activity should be destroyed. If so, it
  ...
  of easyscp or to start a new activity, and which one.
...
When the implementation of easyscp started, in December 2020, file transfer was
at the core of the design, since it is at the heart of easyscp. The first
...
last quarter of 2021, [remotefs](https://crates.io/crates/remotefs) was born:
