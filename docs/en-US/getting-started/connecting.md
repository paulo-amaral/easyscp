...
easyscp can start in three different ways depending on the arguments you pass.

- No arguments: easyscp opens the authentication form, where you provide the
  parameters required to connect to the remote host.
- An address argument: easyscp skips the authentication form and connects
  directly to the remote host.
- A bookmark name with `-b <bookmark-name>`: easyscp resolves the argument as a
...
When easyscp starts without an address, it shows the authentication form. Fill
in the protocol, address, port, username, and password, then connect. easyscp
...
```sh
easyscp scp://omar@192.168.1.31:4022:/tmp
```
...
- You will be prompted for it. This is the default: if you don't use any of the
  methods below, easyscp prompts for the password, like classic tools such as
  `scp` and `ssh`.
...
- Via `sshpass`: provide the password through `sshpass`, for example:
  ...
  ```sh
  sshpass -f ~/.ssh/topsecret.key easyscp cvisintin@192.168.1.31
  ```
