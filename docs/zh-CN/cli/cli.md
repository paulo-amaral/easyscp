...
如果未提供额外参数，easyscp 会显示身份验证表单。如果提供了地址参数或书签名称，easyscp 会跳过该表单并直接连接到远程服务器。当提供地址或书签时，你还可以将本地主机的起始工作目录作为最后一个位置参数提供。
...
将指定 ssh 配置文件中的所有主机作为书签导入到 easyscp 中。如果未提供 `[ssh-config-file]`，则使用默认位置 `~/.ssh/config`。身份文件也会作为 ssh 密钥导入到 easyscp 中。
