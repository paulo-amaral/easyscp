...
easyscp 允许你为应用程序中的多个组件设置颜色。有两种方式可以自定义 easyscp：
...
要通过配置菜单自定义 easyscp，请在认证界面按 `<CTRL+C>` 进入配置，然后按两次 `<TAB>` 到达 `themes` 面板。使用 `<UP>` 和 `<DOWN>` 移动以选择你想更改的样式。

## 导入主题文件

你也可以导入主题文件。你可以从仓库 `themes/` 目录中随 easyscp 附带的某个主题获取灵感，或直接使用它。通过运行以下命令导入主题：

```sh
easyscp theme <theme_file>
```

如果一切正常，easyscp 会确认主题已导入。
