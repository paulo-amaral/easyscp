...
easyscp 可以根据你传入的参数以三种不同的方式启动。

- 不带参数：easyscp 打开认证表单，你在其中提供连接到远程主机所需的参数。
- 带地址参数：easyscp 跳过认证表单，直接连接到远程主机。
- 通过 `-b <bookmark-name>` 传入书签名称：easyscp 将参数解析为已保存的书签并进行连接。重复使用 `-b` 可打开多个书签。

当你提供地址参数或书签名称时，还可以为本地主机提供一个起始工作目录。

## 认证表单

当 easyscp 在不带地址的情况下启动时，会显示认证表单。填写协议、地址、端口、用户名和密码，然后进行连接。连接成功后，easyscp 将打开双面板浏览器。
...
- 系统会提示你输入密码。这是默认方式：如果你不使用下面的任何方法，easyscp 会像 `scp`、`ssh` 等经典工具一样提示你输入密码。
