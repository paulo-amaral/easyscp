...
mod tests {
    ...
    fn test_fs_explorer_sort_by_name() {
        ...
        explorer.set_files(vec![
            ...
            make_fs_entry("src", true),
            make_fs_entry("NOTES.md", false),
            make_fs_entry("SECURITY.md", false),
    ...
    fn test_fs_explorer_sort_by_mtime() {
        ...
        let entry2: File = make_fs_entry("SECURITY.md", false);
        // Create files (files are then sorted by name)
        explorer.set_files(vec![entry1, entry2]);
        explorer.sort_by(FileSorting::ModifyTime);
        // First entry should be "SECURITY.md"
        assert_eq!(explorer.files.get(0).unwrap().name(), "SECURITY.md");
        // Last should be "src"
    ...
    fn test_fs_explorer_sort_by_name_and_dirs_first() {
        ...
        // Last should be "SECURITY.md" (last file for alphabetical ordening)
        assert_eq!(explorer.files.get(9).unwrap().name(), "SECURITY.md");
    }
    ...
    fn test_fs_explorer_sort_by_name_and_dirs_last() {
        ...
        assert_eq!(explorer.files.get(0).unwrap().name(), "Cargo.lock");
        // Last in files should be "SECURITY.md" (last file for alphabetical ordening)
        assert_eq!(explorer.files.get(7).unwrap().name(), "SECURITY.md");
    }
    ...
    fn test_fs_explorer_del_entry() {
        ...
        assert_eq!(explorer.files[0].name(), "NOTES.md");
    ...
    fn test_should_enqueue_and_dequeue_files() {
        ...
        // Enqueue
        explorer.enqueue(Path::new("NOTES.md"), Path::new("NOTES.md"));
        explorer.enqueue(Path::new("docs"), Path::new("docs"));
        // Dequeue
        explorer.dequeue(Path::new("NOTES.md"));
