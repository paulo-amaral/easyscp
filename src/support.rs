//! ## Support
//!
//! this module exposes some extra run modes for easyscp, meant to be used for "support", such as installing themes
...
/// Import theme at provided path into easyscp
...
/// Install latest version of easyscp if an update is available
pub fn install_update() -> Result<String, String> {
    match Update::default()
        ...
        .upgrade()
    {
        Ok(UpdateStatus::AlreadyUptodate) => Ok("easyscp is already up to date".to_string()),
        Ok(UpdateStatus::UpdateInstalled(v)) => {
            ...
            Ok(format!("easyscp has been updated to version {v}"))
        }
        Err(err) => {
            if get_config_client()
                .map(|x| x.get_notifications())
