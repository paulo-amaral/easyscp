//! ## Auto update
//!
//! Automatic update module. This module is used to upgrade the current version of easyscp to the latest available on Github
...
/// Once you're fine with the options, just call the `upgrade()` method to upgrade easyscp.
...
impl Update {
    ...
    /// Returns whether a new version of easyscp is available
...
mod test {
    ...
    #[ignore = "requires a published GitHub release for the configured repository"]
    ...
    fn release() {
        let release: UpdRelease = UpdRelease {
            name: String::from("easyscp 0.7.0"),
    ...
    fn should_accept_release_when_version_is_newer() {
        let release = Release {
            version: String::from("easyscp-999.0.0"),
            body: String::from("notes"),
        };

        let release = Update::check_version(release).unwrap();

        assert_eq!(release.version.as_str(), "easyscp-999.0.0");
