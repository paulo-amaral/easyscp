...
/// easyscp's configuration directory, including the `easyscp` project subdirectory.
///
...
static CONF_DIR: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| Some(std::env::temp_dir().join("easyscp")));

/// easyscp's cache directory, including the `easyscp` project subdirectory.
///
/// The cache directory stays at the platform-native location (`dirs::cache_dir`)
/// on every platform.
#[cfg(not(test))]
static CACHE_DIR: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| dirs::cache_dir().map(|dir| dir.join("easyscp")));
#[cfg(test)]
static CACHE_DIR: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| Some(std::env::temp_dir().join("easyscp")));

/// Resolves easyscp's configuration directory for the current platform,
/// including the `easyscp` project subdirectory.
///
/// The location is platform-specific:
///
/// - **Linux** (and other Unix): `$XDG_CONFIG_HOME/easyscp` (usually `~/.config/easyscp`)
/// - **macOS**: `~/.config/easyscp` (instead of `~/Library/Application Support/easyscp`)
/// - **Windows**: `%USERPROFILE%\.easyscp` (instead of the roaming `%APPDATA%\easyscp`)
#[cfg(not(test))]
fn config_dir() -> Option<PathBuf> {
    #[cfg(macos)]
    {
        // macOS: use ~/.config/easyscp for consistency with Linux and easier
        // CLI access, instead of ~/Library/Application Support/easyscp.
        dirs::home_dir().map(|home| home.join(".config").join("easyscp"))
    }
    #[cfg(win)]
    {
        // Windows: use %USERPROFILE%\.easyscp instead of the roaming %APPDATA%.
        dirs::home_dir().map(|home| home.join(".easyscp"))
    }
    #[cfg(not(any(macos, win)))]
    {
        // Linux and other platforms: keep the XDG-compliant location.
        dirs::config_dir().map(|dir| dir.join("easyscp"))
    }
}
...
fn legacy_config_dir() -> Option<PathBuf> {
    #[cfg(any(macos, win))]
    {
        // Before 1.1.0 the config dir was always `dirs::config_dir()/easyscp`.
        dirs::config_dir().map(|dir| dir.join("easyscp"))
    }
    #[cfg(not(any(macos, win)))]
    {
...
/// Get easyscp config directory path and initialize it.
...
/// Get easyscp cache directory path and initialize it.
/// Returns None if it's not possible to initialize it
...
/// Init a easyscp env dir, creating it if it doesn't already exist.
...
mod tests {
    ...
    fn should_migrate_legacy_config_dir() {
        let base = std::env::temp_dir().join("easyscp-migrate-test-move");
    ...
    fn should_not_migrate_when_new_dir_exists() {
        let base = std::env::temp_dir().join("easyscp-migrate-test-keep");
        let legacy = base.join("legacy");
        let new_dir = base.join("new");
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&legacy).unwrap();
    ...
    fn should_not_migrate_without_legacy_dir() {
        let base = std::env::temp_dir().join("easyscp-migrate-test-none");
        let new_dir = base.join("new");
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn test_system_environment_get_bookmarks_paths() {
        assert_eq!(
            get_bookmarks_paths(Path::new("/home/omar/.config/easyscp/")),
            PathBuf::from("/home/omar/.config/easyscp/bookmarks.toml"),
        );
    }
    ...
    fn test_system_environment_get_config_paths() {
        assert_eq!(
            get_config_paths(Path::new("/home/omar/.config/easyscp/")),
            (
                PathBuf::from("/home/omar/.config/easyscp/config.toml"),
                PathBuf::from("/home/omar/.config/easyscp/.ssh/")
            )
        );
    }
    ...
    fn test_system_environment_get_log_paths() {
        assert_eq!(
            get_log_paths(Path::new("/home/omar/.cache/easyscp/")),
            PathBuf::from("/home/omar/.cache/easyscp/easyscp.log"),
        );
    }
    ...
    fn test_system_environment_get_theme_path() {
        assert_eq!(
            get_theme_path(Path::new("/home/omar/.config/easyscp/")),
            PathBuf::from("/home/omar/.config/easyscp/theme.toml"),
        );
    }
}
