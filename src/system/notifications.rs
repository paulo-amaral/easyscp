...
/// A notification helper which provides all the functions to send the available notifications for easyscp
...
impl Notification {
    ...
    /// Notify a new version of easyscp is available for download
    pub fn update_available<S: AsRef<str>>(version: S) {
        Self::notify(
            "New version available ⬇️",
            format!("easyscp {} is now available for download", version.as_ref()).as_str(),
            None,
        );
    }
    ...
    pub fn update_installed<S: AsRef<str>>(version: S) {
        Self::notify(
        "Update installed 🎉",
            format!("easyscp {} has been installed! Restart easyscp to enjoy the latest version of easyscp 🙂", version.as_ref()).as_str(),
        None,
        );
    }
