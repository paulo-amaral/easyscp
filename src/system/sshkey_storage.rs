...
/// Resolves SSH identity files from easyscp config, SSH config, and standard
...
impl SshKeyStorage {
    ...
    /// Resolve host via easyscp ssh keys storage
    fn resolve_host_in_easyscp_storage(&self, host: &str, username: &str) -> Option<&Path> {
        let key: String = Self::make_mapkey(host, username);
...
impl SshKeyStorageTrait for SshKeyStorage {
    fn resolve(&self, host: &str, username: &str) -> Option<PathBuf> {
        // search in easyscp keys
        if let Some(path) = self.resolve_host_in_easyscp_storage(host, username) {
...
mod tests {
    ...
    fn test_system_sshkey_storage_new() {
        ...
        if let Some(key) = storage.resolve("deskichup", "easyscp") {
...
"#,
    ...
    fn test_system_sshkey_storage_add() {
        ...
        storage.add_key("deskichup", "easyscp", PathBuf::from("/tmp/omar"));
        assert_eq!(
            *storage.resolve("deskichup", "easyscp").unwrap(),
    ...
    fn should_make_mapkey_from_username_and_host() {
        assert_eq!(
            SshKeyStorage::make_mapkey("example.org", "easyscp"),
            "easyscp@example.org"
        );
    }

    #[test]
    fn should_prefer_easyscp_key_over_ssh_config() {
        let rsa_key = test_helpers::create_sample_file_with_content(
            "ssh-rsa  root@8c50fd4c345a",
        );
        let ssh_config_file = test_helpers::create_sample_file_with_content(format!(
