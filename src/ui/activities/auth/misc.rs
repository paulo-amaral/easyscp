...
impl AuthActivity {
    ...
    /// Install latest easyscp version via GUI
    pub(super) fn install_update(&mut self) {
        ...
        // Show outcome
        match result {
            Ok(UpdateStatus::AlreadyUptodate) => self.mount_info("easyscp is already up to date!"),
            Ok(UpdateStatus::UpdateInstalled(ver)) => {
                if self.config().get_notifications() {
                    Notification::update_installed(ver.as_str());
                }
                self.mount_info(format!("easyscp has been updated to version {ver}!"))
            }
            Err(err) => {
                if self.config().get_notifications() {
